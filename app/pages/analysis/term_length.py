...
def term_length_page():
    ...
    st.title("Term Length")
    st.write("Find out who is serving longer than you have been alive!")

    df_house = get_house_data()
    df_senate = get_senate_data()

    age_filter = st.slider(label="Years", min_value=0, max_value=50, value=18)

    filtered_house = df_house[df_house["years_served"] >= age_filter]
    filtered_senate = df_senate[df_senate["years_served"] >= age_filter]

    st.subheader("House of Representatives Members")
    st.dataframe(filtered_house)

    st.subheader("Senate Members")
